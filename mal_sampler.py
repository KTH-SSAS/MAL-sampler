...
class Model:
    def __init__(self, metamodel: dict):
        ...
        for asset_type in self.metamodel:
            if 'n' in self.metamodel[asset_type]:
                ...
                self.n_assets[asset_type] = ProbabilityDistribution(
                    {'distribution': 'Constant', 'n': sys.maxsize})
    ...
    def resolve_inconsistency(self):
        ...
        print(f'# Before forcing associations. Inconsistent assets: {len(inconsistent_assets)}. Assets: {len(self.all_assets())}.')
        counter = 0
        while inconsistent_assets:
            ...
            inconsistent_assets = self.check_consistency()
            print(f'# Before removing assets. Inconsistent assets: {len(inconsistent_assets)}. Assets: {len(self.all_assets())}.')
            for inconsistent_asset in inconsistent_assets:
                self.remove(inconsistent_asset)
                print(
                    f'Removed {inconsistent_asset.name} from the model. Remaining assets: {len(self.all_assets())}.')
            inconsistent_assets = self.check_consistency()
            print(f'# After removing assets. Inconsistent assets: {len(inconsistent_assets)}. Assets: {len(self.all_assets())}.')
            if counter > 10:
